...
STANDALONE_DEST="../firebase-clients/libs/firegrapher"
STANDALONE_STUB="firegrapher"
...
PARSED_CLIENT_VERSION=$(head -5 dist/firegrapher.js | tail -1 | awk -F ' ' '{print $3}')
...
README_VERSION="$(grep '<script src=\"https://cdn.firebase.com/libs/firegrapher/' README.md | awk -F '/' '{print $6}')"
...
if [[ $CHECKED_OUT_BRANCH != "master" ]]; then
  echo "Error: Your firegrapher repo is not on the master branch."
  exit 1
fi

# Pull any changes to the firegrapher repo
git pull origin master
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do git pull from firegrapher repo."
  exit 1
fi
...
if [[ $? -ne 0 ]]; then
  ...
  echo "*** v${VERSION} of firegrapher published to npm ***"
  echo
fi
...
echo "  2) Update the release notes for FireGrapher version ${VERSION} on GitHub"
echo "  3) Update all FireGrapher client version numbers specified in firebase-website to ${VERSION}"
echo "  4) Tweet @FirebaseRelease: 'v${VERSION} of FireGrapher is available https://cdn.firebase.com/libs/firegrapher/$VERSION/firegrapher.min.js Changelog: https://cdn.firebase.com/libs/firegrapher/changelog.txt'"
echo
echo "Done! Woot!"
echo